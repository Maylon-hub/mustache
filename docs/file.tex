...
% ------------------------- CITAÇÕES ABNT -----------------------------
% biblatex-abnt é compatível com a classe article e com versões atuais
% do TeX Live. O processamento da bibliografia deve usar Biber.
\usepackage{csquotes}
\usepackage[
  backend=biber,
  style=abnt,
...
Mais recentemente, \textcite{naldi2022coresg} propuseram o \textbf{CORE-SG}, uma estrutura
...
\begin{enumerate}
  \item \textbf{Alinhamento com a literatura:} O HDBSCAN canônico implementa exatamente os
        algoritmos de Prim/Kruskal exatos descritos por \textcite{campello2013framework} e
        \textcite{campello2015hierarchical}, garantindo 100\% de fidelidade aos resultados
        teóricos publicados.
  ...
  \item \textbf{Superioridade do CORE-SG sobre o RNG já estabelecida:} o artigo original do
        CORE-SG \cite{naldi2022coresg} demonstra, teórica e empiricamente, ganhos de centenas
        a milhares de vezes no cálculo de MSTs baseadas em densidade frente à estratégia baseada
        em RNG, em seus cenários experimentais. Portanto, esta IC não replica essa comparação
        já publicada nem reivindica uma nova demonstração de superioridade sobre o RNG.
\end{enumerate}

Assim, o resultado publicado por Naldi et al. é adotado como fundamentação para a substituição
do motor legado. A avaliação experimental desta IC concentra-se no que é específico desta
implementação: a correção da integração do grafo de suporte, o desempenho do processamento em
lote frente ao HDBSCAN canônico efetivamente disponibilizado no MustaCHE v2, a reprodutibilidade
do pacote e a usabilidade das visualizações. Essa delimitação evita atribuir ao projeto uma
responsabilidade de validação que pertence ao estudo de referência.
...
A integração do CORE-SG seguiu a fundamentação teórica de \textcite{naldi2022coresg}. A
...
processamento em lote e as visualizações coordenadas. A Figura~\ref{fig:datasets} mostra a etapa
anterior de escolha de conjuntos de dados reproduzíveis, enquanto a Figura~\ref{fig:projects}
mostra a persistência e a reabertura das análises, completando o fluxo de uso da ferramenta.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{img/mustache-dashboard-teste.png}
  \caption{Interface principal do MustaCHE v2 após processamento em lote: meta-dendrograma,
           matriz HAI e gráficos de acessibilidade.}
  \label{fig:dashboard}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{img/datasets.png}
  \caption{Catálogo de conjuntos de dados de exemplo e sintéticos, com opção de envio de CSV.}
  \label{fig:datasets}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{img/mustache-projects.png}
  \caption{Histórico local de projetos salvos, com metadados e reabertura sem recomputação.}
  \label{fig:projects}
\end{figure}
...
superioridade do Core-SG sobre o RNG no cálculo de múltiplas MSTs já está estabelecida no estudo
de Naldi et al. (2022); logo, sua revalidação direta não integra o escopo desta IC.
